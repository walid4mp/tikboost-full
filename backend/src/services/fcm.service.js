...
async function sendPush(token, title, body, type = 'info', data = null, opts = {}) {
    ...
    body: JSON.stringify({
      message: {
        ...
        android: {
          priority: 'HIGH',
          ttl: '259200s',
          notification: { channel_id: 'tokaura_general_v2', sound: 'notify', default_sound: false },
        },
      },
    }),
  });
  ...
  if (!response.ok) {
    const errorCode =
      (json && (json.error?.details?.[0]?.errorCode || json.error?.message)) || '';
    const deadToken =
      response.status === 404 ||
      /UNREGISTERED|INVALID_ARGUMENT|NOT_FOUND|registration-token-not-found/i.test(errorCode);
    if (deadToken && opts.userId) {
      try {
        // The stored token is stale (app reinstall / revoked) — drop it so we
        // stop failing forever on this device.
        const prisma = require('../config/db');
        await prisma.user.updateMany({
          where: { id: opts.userId, fcmToken: token },
          data: { fcmToken: null },
        });
        console.warn(`[fcm] cleared stale token for user ${opts.userId}`);
      } catch (err) {
        console.warn(`[fcm] could not clear stale token: ${err.message}`);
      }
    }
    const error = new Error(`FCM send failed: HTTP ${response.status} ${errorCode}`);
    error.status = response.status;
    error.response = json;
    error.deadToken = deadToken;
    throw error;
  }
  return { sent: true, name: json?.name || null };
}

async function sendPushMany(tokens, title, body, type = 'info', data = null, opts = {}) {
  const unique = [...new Set(tokens.filter(Boolean))];
  const results = { sent: 0, failed: 0, skipped: 0 };
  for (let i = 0; i < unique.length; i += 25) {
    const chunk = unique.slice(i, i + 25);
    const settled = await Promise.allSettled(chunk.map((token) => sendPush(token, title, body, type, data, opts)));
