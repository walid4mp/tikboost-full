...
const subscribe = asyncHandler(async (req, res) => {
  ...
  const transactionId = String(req.body?.transactionId || req.body?.reference || '').trim().slice(0, 255);
  const receiptImageData = typeof req.body?.receiptImageData === 'string' ? req.body.receiptImageData.trim() : '';
  if (receiptImageData && receiptImageData.length > 1900000) throw new AppError('صورة الإثبات كبيرة جداً. اختر صورة أصغر.', 400);
  if (transactionId) {
    const duplicate = await prisma.vipSubscription.findFirst({
      where: { reference: transactionId, status: { in: ['PENDING', 'ACTIVE'] } },
      select: { id: true },
    });
    if (duplicate) throw new AppError('رقم المعاملة مستخدم مسبقاً.', 409);
  }
  const row = await prisma.vipSubscription.create({ data: {
    userId: req.user.id,
    priceCents: Number(plan.priceCents),
    currency: settings.vipPro?.currency || 'USD',
    method: String(req.body?.method || 'manual_transfer'),
    reference: transactionId || null,
    planKey: plan.key,
    notes: receiptImageData ? JSON.stringify({ receiptImageData }) : null,
  } });
  ...
  res.status(201).json({ success: true, subscription: { id: row.id, status: row.status, reference: row.reference, createdAt: row.createdAt }, plan, instructions: `أرسل إثبات الدفع للدعم، وسيتم تفعيل ${plan.durationDays} يوماً بعد الموافقة.` });
});
...
const listAdmin = asyncHandler(async (_req, res) => {
  ...
  res.json({ success: true, items: rows.map((row) => {
    ...
    hasReceipt: (() => { try { return !!(row.notes && JSON.parse(row.notes).receiptImageData); } catch (_) { return false; } })(),
  };
  }) });
});

const receiptAdmin = asyncHandler(async (req, res) => {
  const row = await prisma.vipSubscription.findUnique({ where: { id: req.params.id }, select: { notes: true, reference: true } });
...
const actionAdmin = asyncHandler(async (req, res) => {
  ...
  const now = new Date();
  const settings = await getSettings();
  const plan = (settings.vipPro?.plans || []).find(p => p.enabled !== false && ((row.planKey && p.key === row.planKey) || (!row.planKey && p.priceCents === row.priceCents))) || { durationDays: 30 };
...
module.exports = { status, subscribe, listAdmin, receiptAdmin, actionAdmin, grantAdmin, savePlans };
