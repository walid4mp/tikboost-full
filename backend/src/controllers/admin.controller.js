...
const sendNotification = asyncHandler(async (req, res) => {
  ...
  if (target === 'ALL') {
    await broadcast(title, body, type || 'info', payloadData);
    await logAdmin(req.user, 'NOTIFY_ALL', null, { title });
    return res.json({ success: true, count: 'all', audience: 'ALL' });
  }
  ...
  for (const id of ids) await notify(id, title, body, type || 'info', payloadData);
  await logAdmin(req.user, target === 'VIP_PRO' ? 'NOTIFY_VIP_PRO' : 'NOTIFY_USERS', ids.join(','), { title, count: ids.length, audience: target });
  res.json({ success: true, count: ids.length, audience: target });
});
