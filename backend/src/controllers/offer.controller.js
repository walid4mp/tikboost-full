...
function userMatchesOffer(o, user, counters = {}) {
  ...
  if (o.targetGender !== 'ALL' && o.targetGender && o.targetGender !== user.gender) return false;
  if (o.targetCountry !== 'WORLDWIDE' && o.targetCountry && o.targetCountry !== user.countryCode) return false;
...
const purchase = asyncHandler(async (req, res) => {
  ...
  const row = await prisma.purchase.create({ data: { userId: req.user.id, packageId: pkg.id, offerId: offer.id, pointsGiven: points, priceCents: offer.newPriceCents, currency: offer.currency, method: String(req.body?.method || 'manual_transfer'), reference: req.body?.reference ? String(req.body.reference).slice(0,255) : null, status: 'PENDING', notes: `Personal offer: ${offer.title}` } });
  res.status(201).json({ success: true, purchase: row, offer: serializeOffer(offer), instructions: 'تم إنشاء طلب العرض. أرسل إثبات التحويل للدعم ليتم اعتماد النقاط.' });
});
