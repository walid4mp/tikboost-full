...
const validateSignup = [
  ...
  body('referralCode').optional({ values: 'falsy' }).isString().trim().isLength({ min: 4, max: 20 }),
];
...
const validateGoogleLogin = [
  body('email').isEmail().normalizeEmail().withMessage('Invalid email'),
  body('name').optional({ values: 'falsy' }).isString().trim().isLength({ min: 2, max: 50 }),
  body('googleId').isString().trim().isLength({ min: 10, max: 255 }).withMessage('googleId required'),
];

const validateRefresh = [
  body('refreshToken').isString().trim().isLength({ min: 20 }).withMessage('Missing refresh token'),
];

const validateForgotPassword = [
  body('email').isEmail().normalizeEmail().withMessage('Invalid email'),
  body('newPassword').isString().isLength({ min: 8, max: 64 }).withMessage('Password must be 8-64 chars'),
];
...
const googleLogin = asyncHandler(async (req, res) => {
  if (!env.ENABLE_GOOGLE_LOGIN) {
    throw new AppError('Google login is disabled on this server', 503, 'GOOGLE_DISABLED');
  }
...
const refresh = asyncHandler(async (req, res) => {
  ...
  if (!stored || stored.revoked) throw new AppError('Refresh revoked', 401, 'REFRESH_REVOKED');
...
const forgotPassword = asyncHandler(async (req, res) => {
  if (!env.ENABLE_LEGACY_PASSWORD_RESET) {
    throw new AppError('Password reset is disabled until a verified recovery flow is implemented', 501, 'PASSWORD_RESET_DISABLED');
  }
