...
const weekly = asyncHandler(async (req, res) => {
  const settings = await getSettings();
  const profileData = await buildGamificationProfile(req.user.id);
  res.json({ success: true, enabled: settings.engagement?.weeklyEnabled !== false, items: profileData.weekly || [] });
});

const claimWeekly = asyncHandler(async (req, res) => {
  const result = await claimWeeklyChallenge(req.user.id, req.params.key);
  res.json({ success: true, rewardPoints: result.rewardPoints.toString(), item: result.item, balance: result.user.points.toString() });
});
