...
r.use((req, _res, next) => {
  const p = req.path;
  const map = p.startsWith('/users') ? 'users' : p.startsWith('/campaigns') ? 'campaigns' : p.startsWith('/purchases') ? 'purchases' : p.startsWith('/reports') ? 'reports' : p.startsWith('/notifications') ? 'notifications' : p.startsWith('/logs') ? 'logs' : p.startsWith('/settings/ads') ? 'ads' : p.startsWith('/settings/payments') ? 'purchases' : p.startsWith('/settings/rewards') ? 'rewards' : p.startsWith('/packages') ? 'packages' : p.startsWith('/offers') ? 'offers' : p.startsWith('/wheel') ? 'rewards' : p.startsWith('/stats') ? 'dashboard' : p.startsWith('/analytics') ? 'analytics' : p.startsWith('/admins') ? 'admins' : p.startsWith('/settings') ? 'settings' : p.startsWith('/vip') ? 'rewards' : p.startsWith('/password-reset-requests') ? 'users' : 'dashboard';
...
r.get('/purchases/:id/receipt', purchaseCtrl.purchaseReceipt);
...
r.get('/vip/subscriptions/:id/receipt', require('../controllers/vip.controller').receiptAdmin);
...
r.post('/stats/recalculate-revenue', ctrl.recalculateRevenue);
