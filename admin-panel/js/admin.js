...
if (store.access && store.me && store.me.role) { applyAdminNavPermissions(); enterApp(); }
...
document.getElementById('notifySend').addEventListener('click', async () => {
  try {
    await api('/admin/notifications/send', { method:'POST', body: {
      userIds: Array.from(document.getElementById('notifyTarget').selectedOptions).map(o => o.value).filter(Boolean),
...
async function deletePackage(id) {
  ...
  try { await api(`/admin/packages/${id}`, {method:'DELETE'}); toast('تم حذف الباقة', 'success'); loadPackages(); } catch(e) { toast(e.message, 'err'); }
}
...
async function loadVip() {
  try { const data=await api('/admin/vip/subscriptions'); const tbody=document.querySelector('#vipTable tbody'); tbody.innerHTML=''; (data.items||[]).forEach(x=>tbody.insertAdjacentHTML('beforeend',`<tr><td>${esc(x.user?.name||'-')}<br><small>${esc(x.user?.email||'')}</small></td><td>$${(Number(x.priceCents||0)/100).toFixed(2)}</td><td>${esc(x.method||'-')}</td><td>${esc(x.status)}</td><td>${new Date(x.createdAt).toLocaleString()}</td><td>${x.status==='PENDING'?`<button class="act success vip-action" data-id="${x.id}" data-decision="APPROVED">تفعيل 30 يوم</button> <button class="act danger vip-action" data-id="${x.id}" data-decision="REJECTED">رفض</button>`:'—'}</td></tr>`)); document.querySelectorAll('.vip-action').forEach(b=>b.onclick=async()=>{try{await api(`/admin/vip/subscriptions/${b.dataset.id}/action`,{method:'POST',body:{decision:b.dataset.decision}});toast('تم تحديث الطلب','success');loadVip();}catch(e){toast(e.message,'err');}}); } catch(e){toast(e.message,'err');}
}
...
async function loadOffers() {
  ...
  try {
    const pkgData = await api('/admin/packages');
...
function editOffer(id) {
  ...
  document.getElementById('offerStart').value = o.startsAt || '';
  document.getElementById('offerEnd').value = o.endsAt || '';
...
async function saveOffer() {
  ...
  const body = {
    ...
    startsAt: document.getElementById('offerStart').value || null,
    endsAt: document.getElementById('offerEnd').value || null,
    ...
    (data.items || []).forEach(r => {
      ...
      tbody.insertAdjacentHTML('beforeend', `
        <tr>
          ...
          <td>
            ${status === 'PENDING' && !expired ? `<button class="primary" onclick="revealResetCode('${r.id}')">إظهار الرمز</button>` : ''}
...
async function revealResetCode(id) {
  try {
    ...
    if (el) el.innerHTML = `<b>${esc(data.code)}</b> <button onclick="copyText('${esc(data.code)}')">نسخ</button>`;
  } catch (e) { toast(e.message, 'err'); }
}
...
async function copyText(value) {
  try { await navigator.clipboard.writeText(value); toast('تم نسخ الرمز', 'success'); }
  catch { toast('تعذر النسخ تلقائيًا', 'err'); }
}
