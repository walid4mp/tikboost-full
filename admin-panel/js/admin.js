...
function switchPage(name) {
  ...
  document.getElementById('pageTitle').textContent = ({
    ...
    purchases: 'المدفوعات', paymentSettings: 'طرق الدفع', reports: 'البلاغات', notify: 'إرسال إشعار',
  ...
  ({ dashboard: loadDashboard, users: loadUsers, campaigns: loadCampaigns,
     purchases: loadPurchases, paymentSettings: loadPaymentSettings, reports: loadReports, notify: loadNotify,
...
function applyAdminNavPermissions() {
  ...
  document.querySelectorAll('.nav-btn').forEach(btn => {
    ...
    else if (page === 'paymentSettings' && !permissions.includes('purchases')) btn.classList.add('hidden');
    else if (!permissions.includes(page) && !['dashboard','paymentSettings'].includes(page)) btn.classList.add('hidden');
  });
}
...
function setDefaultStatsDates(){
  const to = new Date(), from = new Date(Date.now()-29*86400000);
  const f=document.getElementById('statsFrom'), t=document.getElementById('statsTo');
  if(f && !f.value) f.value=isoDate(from); if(t && !t.value) t.value=isoDate(to);
}
async function loadDashboard() {
  setDefaultStatsDates();
  try {
    const from=document.getElementById('statsFrom')?.value || '';
    const to=document.getElementById('statsTo')?.value || '';
    const qs=`?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
    const [stats, top, series]=await Promise.all([api('/admin/stats'+qs),api('/admin/stats/top-users'),api('/admin/stats/chart?days=30')]);
    const st=stats.stats;
    document.getElementById('kpiUsers').textContent=st.users;
    document.getElementById('kpiActiveDay').textContent=st.activeUsers?.day ?? 0;
    document.getElementById('kpiPayingUsers').textContent=st.payingUsers ?? 0;
    document.getElementById('kpiRevenue').textContent='$'+(Number(st.revenueCents||0)/100).toFixed(2);
    document.getElementById('kpiActive7').textContent=st.activeUsers?.days7 ?? 0;
    document.getElementById('kpiActive30').textContent=st.activeUsers?.days30 ?? 0;
    document.getElementById('kpiPurchasesPeriod').textContent=st.purchases?.period ?? 0;
    document.getElementById('kpiPendingPurchases').textContent=st.purchases?.pending ?? 0;
    const labels=series.series.map(d=>d.date.slice(5));
    const ctx=document.getElementById('chartActivity').getContext('2d'); if(chartActivity) chartActivity.destroy();
    chartActivity=new Chart(ctx,{type:'line',data:{labels,datasets:[
      {label:'تسجيلات',data:series.series.map(d=>d.signups),tension:.35,borderColor:'#FF3B5C',backgroundColor:'rgba(255,59,92,.12)',fill:true},
      {label:'مهام',data:series.series.map(d=>d.tasks),tension:.35,borderColor:'#2D7BFF',backgroundColor:'rgba(45,123,255,.10)',fill:true},
      {label:'أرباح $',data:series.series.map(d=>(Number(d.revenueCents||0)/100)),tension:.35,borderColor:'#22C55E',backgroundColor:'rgba(34,197,94,.08)',fill:false},
      {label:'نشاط',data:series.series.map(d=>d.activeLogins||0),tension:.35,borderColor:'#F59E0B',backgroundColor:'rgba(245,158,11,.08)',fill:false},
    ]},options:{responsive:true,plugins:{legend:{labels:{color:'#F2F2F5'}}},scales:{x:{ticks:{color:'#8C8C99'},grid:{color:'#26262F'}},y:{ticks:{color:'#8C8C99'},grid:{color:'#26262F'},beginAtZero:true}}}});
    const topEl=document.getElementById('topUsers'); topEl.innerHTML='';
    (top.items||[]).forEach((u,i)=>topEl.insertAdjacentHTML('beforeend',`<div class="row"><div style="width:28px;height:28px;border-radius:8px;background:rgba(255,59,92,.15);color:#FF3B5C;display:grid;place-items:center;font-weight:900">${i+1}</div><div style="flex:1"><div><b>${esc(u.name||'-')}</b></div><small>${esc(u.email||'')} • ${esc(u.totalEarned||0)} نقطة مكتسبة</small></div><small>${u.lastLoginAt?new Date(u.lastLoginAt).toLocaleString():'لا يوجد دخول'}</small></div>`));
    // recent active users: use users endpoint sorted by lastLoginAt client-side
    const users=await api('/admin/users?limit=100&page=1&status=ACTIVE');
    const active=(users.items||[]).filter(u=>u.lastLoginAt).sort((a,b)=>new Date(b.lastLoginAt)-new Date(a.lastLoginAt)).slice(0,12);
    const activeEl=document.getElementById('activeUsersSummary'); activeEl.innerHTML=active.length?active.map(u=>`<div class="row"><div style="flex:1"><b>${esc(u.name||'-')}</b><small style="display:block;color:#8C8C99">${esc(u.email||'')}</small></div><div><b>${new Date(u.lastLoginAt).toLocaleString()}</b><small style="display:block;color:#8C8C99">${esc(u.status)}</small></div></div>`).join(''):'<div class="muted">لا توجد بيانات نشاط حتى الآن.</div>';
  } catch(e){toast(e.message,'err');}
}
document.getElementById('statsApply')?.addEventListener('click',loadDashboard);
document.getElementById('recalculateRevenue')?.addEventListener('click',async()=>{try{const r=await api('/admin/stats/recalculate-revenue',{method:'POST'});toast(`تمت إعادة الحساب: $${(Number(r.revenueCents||0)/100).toFixed(2)} من ${r.purchases||0} عملية`,'success');loadDashboard();}catch(e){toast(e.message,'err');}});
    ...
    data.items.forEach(u => {
      ...
      tbody.insertAdjacentHTML('beforeend', `
        <tr>
          ...
          <td>${new Date(u.createdAt).toLocaleDateString()}<br><small>${u.lastLoginAt ? 'نشط: '+new Date(u.lastLoginAt).toLocaleString() : 'لم يسجل دخول'}</small></td>
          <td>
...
function paymentMethodRow(method, index) {
  const m = method || {};
  return `<div class="card payment-method-row" style="padding:14px;margin-bottom:10px">
    <div class="grid-2 contact-grid">
      <div><label>المعرف</label><input class="pm-key" value="${esc(m.key || `method_${index+1}`)}" placeholder="baridimob"></div>
      <div><label>الاسم الظاهر</label><input class="pm-label" value="${esc(m.label || '')}" placeholder="BaridiMob"></div>
      <div><label>العملة</label><input class="pm-currency" value="${esc(m.currency || 'USD')}" placeholder="DZD / USD / USDT"></div>
      <div><label>نوع الدفع</label><select class="pm-type"><option value="manual" ${(m.type || 'manual')==='manual'?'selected':''}>تحويل يدوي + إثبات</option><option value="paypal" ${(m.type || '')==='paypal'?'selected':''}>PayPal</option></select></div>
      <div><label>الأيقونة الاحتياطية</label><input class="pm-icon" value="${esc(m.icon || '💳')}" placeholder="💳"></div>
      <div><label>صورة الوسيلة</label><select class="pm-logo"><option value="">بدون صورة</option><option value="baridimob" ${m.logoKey==='baridimob'?'selected':''}>BaridiMob</option><option value="binance_pay" ${m.logoKey==='binance_pay'?'selected':''}>Binance Pay</option><option value="redotpay" ${m.logoKey==='redotpay'?'selected':''}>RedotPay</option><option value="paypal" ${m.logoKey==='paypal'?'selected':''}>PayPal</option><option value="usdt_trc20" ${m.logoKey==='usdt_trc20'?'selected':''}>USDT TRC20</option><option value="manual_transfer" ${m.logoKey==='manual_transfer'?'selected':''}>Manual</option></select></div>
      <div><label>رقم / عنوان / بريد الدفع</label><input class="pm-address" value="${esc(m.walletAddress || '')}" placeholder="رقم BaridiMob / Binance ID / PayPal Email / USDT Address"></div>
      <div><label>اسم الحساب</label><input class="pm-account" value="${esc(m.accountName || '')}" placeholder="اسم المستفيد"></div>
      <div><label>رابط الدفع الخارجي (اختياري)</label><input class="pm-url" value="${esc(m.externalUrl || '')}" placeholder="https://paypal.me/..."></div>
      <div><label>الشبكة (للعملات الرقمية)</label><input class="pm-network" value="${esc(m.network || '')}" placeholder="TRC20"></div>
      <div><label>الترتيب</label><input class="pm-sort" type="number" min="0" value="${Number.isFinite(Number(m.sortOrder)) ? Number(m.sortOrder) : index}"></div>
      <div><label>الحد الأدنى</label><input class="pm-min" type="number" min="0" step="0.01" value="${m.minAmountCents ? Number(m.minAmountCents)/100 : 0}"></div>
      <div><label>الحد الأقصى (0 = بدون حد)</label><input class="pm-max" type="number" min="0" step="0.01" value="${m.maxAmountCents ? Number(m.maxAmountCents)/100 : 0}"></div>
      <div><label>رسوم %</label><input class="pm-fee" type="number" min="0" max="100" step="0.01" value="${m.feePercent || 0}"></div>
      <div style="grid-column:1/-1"><label>تعليمات الدفع للمستخدم</label><textarea class="pm-instructions" rows="3">${esc(m.instructions || '')}</textarea></div>
    </div>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px">
      <label><input class="pm-enabled" type="checkbox" ${m.enabled !== false ? 'checked' : ''}> مفعلة للمستخدمين</label>
      <button type="button" class="danger payment-remove">حذف</button>
    </div>
  </div>`;
}
function readPaymentMethodsEditor() {
  return [...document.querySelectorAll('#paymentMethodsEditor .payment-method-row')].map(row => ({
    key: row.querySelector('.pm-key').value.trim().toLowerCase(),
    label: row.querySelector('.pm-label').value.trim(),
    icon: row.querySelector('.pm-icon').value.trim(),
    logoKey: row.querySelector('.pm-logo')?.value || '',
    type: row.querySelector('.pm-type').value,
    currency: row.querySelector('.pm-currency').value.trim().toUpperCase() || 'USD',
    walletAddress: row.querySelector('.pm-address').value.trim(),
    accountName: row.querySelector('.pm-account').value.trim(),
    externalUrl: row.querySelector('.pm-url').value.trim(),
    network: row.querySelector('.pm-network').value.trim(),
    sortOrder: Math.max(0, Number(row.querySelector('.pm-sort').value || 0)),
    minAmountCents: Math.round(Math.max(0, Number(row.querySelector('.pm-min').value || 0)) * 100),
    maxAmountCents: Math.round(Math.max(0, Number(row.querySelector('.pm-max').value || 0)) * 100),
    feePercent: Math.max(0, Math.min(100, Number(row.querySelector('.pm-fee').value || 0))),
    instructions: row.querySelector('.pm-instructions').value.trim(),
    enabled: row.querySelector('.pm-enabled').checked,
  })).filter(x => x.key && x.label);
}
function bindPaymentEditor() {
  document.querySelectorAll('.payment-remove').forEach(btn => btn.onclick = () => btn.closest('.payment-method-row')?.remove());
}
async function loadPaymentSettings() {
  try {
    const data = await api('/admin/settings/payments');
    const box = document.getElementById('paymentMethodsEditor');
    box.innerHTML = (data.payments?.methods || []).map(paymentMethodRow).join('');
    bindPaymentEditor();
  } catch(e) { toast(e.message, 'err'); }
}
document.getElementById('paymentMethodAdd')?.addEventListener('click', () => {
  const box = document.getElementById('paymentMethodsEditor');
  const n = box.querySelectorAll('.payment-method-row').length + 1;
  box.insertAdjacentHTML('beforeend', paymentMethodRow({key:`method_${n}`,label:'طريقة دفع جديدة',icon:'💳',enabled:true,logoKey:''}, n-1));
  bindPaymentEditor();
});
document.getElementById('paymentSettingsSave')?.addEventListener('click', async () => {
  try {
    const methods = readPaymentMethodsEditor();
    if (!methods.length) throw new Error('أضف طريقة دفع واحدة على الأقل.');
    await api('/admin/settings/payments', {method:'PUT', body:{payments:{methods}}});
    toast('تم حفظ طرق الدفع','success');
  } catch(e) { toast(e.message,'err'); }
});
async function showPaymentReceipt(item, type='purchase') {
  try {
    const endpoint = type === 'vip'
      ? `/admin/vip/subscriptions/${item.id}/receipt`
      : `/admin/purchases/${item.id}/receipt`;
    const data = await api(endpoint);
    const img = data.receiptImageData;
    const tx = data.transactionId || item?.transactionId || '-';
    const modal = document.createElement('div');
    modal.className='modal';
    modal.innerHTML=`<div class="modal-card" style="max-width:760px"><button class="modal-close">✕</button><h3>إثبات الدفع</h3><p><b>رقم المعاملة:</b> ${esc(tx)}</p>${img ? `<img src="${img}" style="display:block;max-width:100%;max-height:70vh;margin:auto;border-radius:14px;border:1px solid var(--border)">` : '<p class="muted">لا توجد صورة إثبات.</p>'}</div>`;
...
async function loadPurchases() {
  ...
  data.items.forEach(p => {
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr>
        ...
        <td><b>${esc(p.paymentMethod?.label || p.method || '-')}</b><br><small>${esc(p.paymentMethod?.currency || p.currency || '')}</small><br><b>${esc(p.transactionId || '-')}</b><br>${p.hasReceipt ? `<button class="act receipt-btn" data-id="${p.id}">عرض الإثبات</button>` : '<small class="muted">بدون صورة</small>'}</td>
  ...
  document.querySelectorAll('#purchasesTable .receipt-btn').forEach(b => b.onclick = () => {
    const item = data.items.find(x => String(x.id) === String(b.dataset.id));
    if (item) showPaymentReceipt(item, 'purchase');
  });
}
...
document.getElementById('notifySend').addEventListener('click', async () => {
  try {
    ...
    if (!title || !body) throw new Error('أدخل عنوان الإشعار ونصه');

    const result = await api('/admin/notifications/send', { method:'POST', body: {
      audience, userIds: selected, title, body,
      type: document.getElementById('notifyType').value,
    }});

    const push = result.push || null;
    const statusEl = document.getElementById('notifyPushStatus');
    if (statusEl) {
      if (push) {
        statusEl.textContent = result.count === 'all'
          ? `تم إنشاء الإشعار لـ ${result.deliveredTo || 0} مستخدم • FCM: ${push.sent || 0} أُرسل، ${push.failed || 0} فشل، ${push.skipped || 0} بدون جهاز مسجل.`
          : `تم الإرسال إلى ${result.count} مستخدم • FCM: ${push.sent || 0} أُرسل، ${push.failed || 0} فشل، ${push.skipped || 0} بدون جهاز مسجل.`;
      } else {
        statusEl.textContent = 'تم إنشاء الإشعار داخل التطبيق. لم تتوفر إحصائيات FCM.';
      }
    }
...
async function loadVip() {
  try {
    ...
    document.querySelectorAll('.vip-receipt').forEach(b=>b.onclick=()=>{const item=(data.items||[]).find(x=>String(x.id)===String(b.dataset.id));if(item)showPaymentReceipt(item, 'vip');});
