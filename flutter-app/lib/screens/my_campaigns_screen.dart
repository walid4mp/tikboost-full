  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async { await SoundService.instance.playTap(); context.push('/create'); },
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      if (c['status'] == 'ACTIVE' || c['status'] == 'PAUSED')
                        Row(children: [
                          ...
                          if (c['status'] == 'PAUSED')
                            TextButton.icon(
                              onPressed: () async { await SoundService.instance.playTap(); await action(c['id'], 'pause'); },
                              icon: const Icon(Icons.play_arrow, size: 18),
                              label: const Text('استئناف')),
                          TextButton.icon(
                            onPressed: () async { await SoundService.instance.playTap(); await action(c['id'], 'cancel'); },
