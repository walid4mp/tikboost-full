...
class _ReferralsScreenState extends ConsumerState<ReferralsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final code = d['code'] ?? '';
    final link = d['link'] ?? '';
    final downloadUrl = '${d['downloadUrl'] ?? AppConfig.downloadUrl ?? ''}';
    return Scaffold(
          ...
          Card(child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                IconButton(
                  onPressed: () {
                    SoundService.instance.play('notify.wav'); Clipboard.setData(ClipboardData(text: code));
              ...
              if (link.toString().isNotEmpty) ...[
                ...
                Row(children: [
                  Expanded(child: SelectableText(link, style: const TextStyle(color: AppColors.textMuted, fontSize: 12))),
                  IconButton(onPressed: () { SoundService.instance.play('notify.wav'); Clipboard.setData(ClipboardData(text: link)); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم نسخ رابط الإحالة'))); }, icon: const Icon(Icons.copy, color: AppColors.blue)),
                ]),
              ],
              if (downloadUrl.trim().isNotEmpty) ...[
                const SizedBox(height: 8),
                Row(children: [
                  Expanded(child: SelectableText(downloadUrl, style: const TextStyle(color: AppColors.textMuted, fontSize: 12))),
                  IconButton(onPressed: () { SoundService.instance.play('notify.wav'); Clipboard.setData(ClipboardData(text: downloadUrl)); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم نسخ رابط تحميل التطبيق'))); }, icon: const Icon(Icons.download, color: AppColors.blue)),
                ]),
              ],
            ]),
          )),
